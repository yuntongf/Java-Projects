...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        // insert pixel to borders while keeping other pixels
        for (int row = 0; row < h + 2 * borderWidth; row++) {
            for (int col = 0; col < w + 2 * borderWidth; col++) {
                if (col < borderWidth || col > borderWidth + w - 1 || row < borderWidth
                        || row > borderWidth + h - 1) {
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                int newColor = (int) Math.round(avg);
                tgt[row][col] = new Pixel(newColor, newColor, newColor);
            }
        }
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                Pixel p1 = src1[row][col];
                Pixel p2 = src2[row][col];

                // scale each color
                int r = weightedAverage(alpha, p1.getRed(), p2.getRed());
                int g = weightedAverage(alpha, p1.getGreen(), p2.getGreen());
                int b = weightedAverage(alpha, p1.getBlue(), p2.getBlue());
